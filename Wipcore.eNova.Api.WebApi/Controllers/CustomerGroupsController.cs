...
namespace Wipcore.eNova.Api.WebApi.Controllers
{
    ...
    public class CustomerGroupsController : EnovaApiController
    {
        ...
        {
            return _objectService.Get<EnovaCustomerGroup>(requestContext, getParameters);
        }

        [HttpGet("{identifier}")]
        public IDictionary<string, object> Get(ContextModel requestContext, GetParametersModel getParameters, string identifier)
        {
            return _objectService.Get<EnovaCustomerGroup>(requestContext, getParameters, identifier);
        }
    }
}
